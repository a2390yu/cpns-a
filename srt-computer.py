...
def parse_args():
    ...
    parser.add_argument("--time_test", "-tt", nargs=2, default=0) # num_iters, percent initial marking size
...
def calculate_reachable(start_marking, end_marking, net, lim_reachability=True, algs=["exact"], take_first_n=1):
    ...
    if "min" in algs:
        print("\nATLEASTREACHABLE\n===========================")
        start = time()
        (max_reachable_mass, count_iters) = net.max_min(sm=start_marking, em=end_marking, lim_reachability=lim_reachability)
        end = time()
        times["min"] = end-start
...
def time_test(net, algs, lim_reachability=True, percent=0.5, num_iters=10):
    ...
    for _ in range(num_iters):
        ...
        (times, count_iters) = calculate_reachable(sm, em, net, lim_reachability=lim_reachability, algs=algs, take_first_n=1)
        alg_sums = {alg: alg_sums[alg]+times[alg] for alg in alg_sums.keys()}

        if "min" in algs:
            alr_total_iters += count_iters

    if "min" in algs:
        alr_iter_time = alg_sums['min']/alr_total_iters
    alg_sums = {alg: alg_sums[alg]/num_iters for alg in alg_sums.keys()}
    if "min" in algs:
        alg_sums["ALR iter"] = alr_iter_time
